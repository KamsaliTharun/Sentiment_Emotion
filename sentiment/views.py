    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if sentiment > 0:
                ...
                emotion = 'Neutral'
            args = {'tweet': tweet, 'sentiment': emotion}
            return render(request, 'home/sentiment_type_result.html', args)
    else:
        form = Sentiment_Typed_Tweet_analyse_form()
...
def sentiment_analysis_import(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            handle = form.cleaned_data['sentiment_imported_tweet']
            # Add your code here to fetch and analyze the tweets
            # and determine the sentiment/emotion
            # Update the 'sentiment' variable based on the sentiment analysis
            sentiment = 'Neutral'  # Replace with your sentiment analysis logic
            args = {'handle': handle, 'sentiment': sentiment}
            return render(request, 'home/sentiment_import_result.html', args)
    else:
        form = Sentiment_Imported_Tweet_analyse_form()
